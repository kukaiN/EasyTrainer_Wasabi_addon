...
def get_optimizer(args, trainable_params):
    ...
    if optimizer_type == "CAME".lower():
        try:
            ...
            raise ImportError("CAME hasn't been installed run pip install came-pytorch in your venv")
        print(f"Using CAME, don't blame if it breaks | {optimizer_kwargs}")
...
def get_scheduler_fix(args, optimizer: Optimizer, num_processes: int):
    ...
    def lr_lambda_rex(scheduler_steps: int,):
...
def get_noise_noisy_latents_and_timesteps_TA(args, noise_scheduler, latents, current_step, loss_map, use_TA=False):#current_timesteps,
    ...
    if args.noise_offset: # custom noise function, more elaborate with random strength
        if args.noise_offset_random_strength:
            ...
            noise_start = 0.0
            noise_offset = noise_start + ((args.noise_offset - noise_start) * (current_step / args.max_train_steps))
    ...
    if args.multires_noise_iterations:
        ...
        # this is the same as default noise
        noise = custom_train_functions.pyramid_noise_like(
    ...
    #selected_file_path = base_path + str(run_number) + extension
    ...
    return noise, noisy_latents, timesteps, huber_c, stat_dict # current_timesteps, run_number,loss_map,
