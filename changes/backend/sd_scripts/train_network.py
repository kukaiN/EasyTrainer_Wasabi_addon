...
class NetworkTrainer:
    ...
    def train(self, args):
        ...
        if args.deepspeed:
            ...
            if train_text_encoder:
                if len(text_encoders) > 1:
                    ...
                    if hasattr(args, "te2_disable") and args.te2_disable:
                        print("disabled TE2 training")
                        text_encoder = text_encoders = [accelerator.prepare(t_enc) if i != 1 else t_enc for i, t_enc in enumerate(text_encoders)]
                    else:
                        text_encoder = text_encoders = [accelerator.prepare(t_enc) for t_enc in text_encoders]
                else:
        ...
        if args.gradient_checkpointing:
            ...
            for i, t_enc in enumerate(text_encoders):
                ...
                if train_text_encoder:
                    ...
                    if i!=1:
                        t_enc.text_model.embeddings.requires_grad_(True)
                    elif i == 1 and hasattr(args, "te2_disable") and args.te2_disable:
                        print("disabled TE2 training, disabling require grad")
                        pass
                    else:
                        t_enc.text_model.embeddings.requires_grad_(True)
        ...
        save_lossmap = False
        if hasattr(args, "save_lossmap") and args.save_lossmap:
            save_lossmap = args.save_lossmap
        ...
        if use_user_config:
            ...
            if True:
                ...
                for subset_name, tag_freq in dataset.tag_frequency.items():
                    tag_frequency[subset_name] = filter_dict(tag_freq, subset_name)
            
                for i in range(len(datasets_metadata)):
                    for subset_name, tag_freq in datasets_metadata[i]["tag_frequency"].items():
                        datasets_metadata[i]["tag_frequency"] = filter_dict(tag_freq, subset_name)
        ...
        if accelerator.is_main_process:
            ...
            if args.wandb_run_name:
                ...
                init_kwargs["wandb"] = {"name": args.output_name}
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(training_model):
                    ...
                    if time_attention:
                        ...
                        if args.logging_dir is not None and save_lossmap: # additional logs added by wasabi
                            ...
                            if (global_step < 100 and global_step%10==0) or (global_step> 99 and global_step%100==0):
                                if global_step == 0:
