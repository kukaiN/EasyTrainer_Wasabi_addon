...
def train(args):
    ...
    real_global_step = 0
    if args.resume_from_huggingface: # added by wasabi
        optimizer_state_dict = optimizer.state_dict()
        # Check each state dictionary for the global step
        for param_group in optimizer_state_dict['param_groups']:
            if 'step' in param_group:
                real_global_step = param_group['step']
                break

        if not real_global_step:
            # Sometimes it's stored in the 'state' part of the optimizer's state_dict
            for state in optimizer_state_dict['state'].values():
                if isinstance(state, dict) and 'step' in state:
                    real_global_step = state['step']
                    break
        print(f"Global step: {real_global_step}")
    loss_map = {}
    stat_dict = None
    default_loss = 0.2
    for ts in range(1, 1000):  # Assuming timesteps from 1 to 999
        if ts not in loss_map:
            loss_map[ts] = default_loss
    
    #args.loss_type = "l2"
    
    debias_limit_val = 0
    if hasattr(args, "debias_limit_val") and args.debias_limit_val:
        debias_limit_val = args.debias_limit_val
        
    time_attention = False
    if hasattr(args, "time_attention") and args.time_attention:
    ...
    if accelerator.is_main_process:
        ...
        train_config = {
            "data_count":train_dataset_group.num_train_images,
            "batch_size": ', '.join([str(d.batch_size) for d in train_dataset_group.datasets]),
            "grad_accum_steps": args.gradient_accumulation_steps,
            "total_steps": args.max_train_steps,
            "optimizer": args.optimizer_type,
            "scheduler": args.lr_scheduler,
            "Unet-lr": args.learning_rate,
            "TE1-lr": args.learning_rate_te1,
            "TE2-lr": args.learning_rate_te2,
            "device_name": torch.cuda.get_device_name(0),
            "tag_dropout_rate": args.caption_tag_dropout_rate,
            "multires_noise_iter":args.multires_noise_iterations,
            "multires_discount": args.multires_noise_discount,
            "debiased_estimation_loss":args.debiased_estimation_loss,
            
            "start_global_step": real_global_step,
            }
        if hasattr(args, "time_attention") and args.time_attention:
                train_config["time_attention"] = args.time_attention
        else:
            pass
            #train_config["time_attention"] = False
        if hasattr(args, "debias_limit_val") and args.debias_limit_val:
                train_config["debias_limit_val"] = args.debias_limit_val
        else:
            pass
            # train_config["debias_limit_val"] = 0
        ...
        if args.ip_noise_gamma:
            train_config["ip_noise_gamma"] = args.ip_noise_gamma
        else:
             train_config["ip_noise_gamma"] = 0
        if args.min_snr_gamma:
            train_config["min_snr_gamma"] = args.min_snr_gamma
        else:
             train_config["min_snr_gamma"] = 0
          
        accelerator.init_trackers("finetuning" if args.log_tracker_name is None else args.log_tracker_name,config=train_config, init_kwargs=init_kwargs)
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*training_models):
                ...
                if (
                    ...
                    if args.debiased_estimation_loss:
                        loss = apply_debiased_estimation(loss, timesteps, noise_scheduler, debias_limit_val)
                    ...
                    if time_attention:
                        loss_map = update_loss_map_ema_edit(loss_map, loss, timesteps, args.max_train_steps)
                    
                    if args.logging_dir is not None:
                        # added by wasabi
                        #added_logs = {"timesteps" : [int(i) for i in timesteps],
                        #              "individual_loss": loss[:, 0, 0, 0].detach().tolist()
                        #}
                        #accelerator.log(added_logs, step=global_step)
                        grads = np.array([loss_map.get(loss_t, 1) for loss_t in range(1, 1000)])
                        grad_sum = np.sum(grads)
                        loss_map_val = grads/grad_sum
                        timestep_list = [t.item() for t in timesteps]
                        ...
                        log_dict = {"loss_map/min":min(grads),
                                 "loss_map/max":max(grads),
                                 "loss_map/maxminratio":max(grads)/min(grads),
                                 "sample_time/min":min(timestep_list),
                                 "sample_time/max":max(timestep_list)
                                 #"sample_time/avg":np.mean(timestep_list),
                                }
                        ...
                        if (global_step < 100 and global_step%10==0) or (global_step> 99 and global_step%50==0):
                            if global_step == 0:
                                # apparently the plt figure reference is not deleted even with explicit del line
                                # so we opt to keeping the same figure, but redrawing on top of it
                                # https://stackoverflow.com/questions/21884271/warning-about-too-many-open-figures
                                fig, ax = plt.subplots(figsize=(13, 8)) # golden ratio plot
                            ax.stairs(grads/grad_sum, range(0,1000), baseline=0, fill=True)
                            ax.set_xlim([1, 1000])
                            ax.set_ylabel("Probability")
                            ax.set_xlabel("Time, T")
                            ax.set_title("Density plot across T~[1,1000]")
                            ax.grid(True)
            ...
            if accelerator.sync_gradients:
                ...
                if args.save_every_n_steps is not None and global_step % args.save_every_n_steps == 0:
                    ...
                    if args.logging_dir is not None and args.resume_from_huggingface: # added by wasabi, only for checkpoint
                        accelerator.log({"saved_global_step": real_global_step}, step=global_step)
